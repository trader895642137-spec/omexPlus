    ...
    const opportunities = sortedStrategies.filter(strategy => {
        ...
        if (ignoreStrategyList.find(ignoreStrategyObj => {
            ...
            if (ignoreStrategyObj.type !== 'ALL' && ignoreStrategyObj.type !== strategy.strategyTypeTitle)
                    return false
            // if(strategy.strategyTypeTitle==='COVERED' && ignoreStrategyObj.type==='COVERED'){
            //     console.log(333)
            // }
...
const calcBOX_BULL_PUTStrategies = (list, {priceType, expectedProfitPerMonth, min_time_to_settlement=0, max_time_to_settlement=Infinity, minVol=CONSTS.DEFAULTS.MIN_VOL, expectedProfitNotif=false, ...restConfig}) => {
            ...
            const _enrichedList = optionListOfSameDate.map(option => {
                ...
                let allPossibleStrategies = optionListWithHigherStrikePrice.reduce( (_allPossibleStrategies, higherStrikeOption) => {
                    ...
                    const strategyObj = {
                        ...
                        strategyTypeTitle: "BOX_BULL_PUT",
                        expectedProfitNotif,
                        expectedProfitPerMonth,
                        name: createStrategyName([option, higherStrikeOption]),
                        profitPercent
        ...
        let enrichedListOfStock = Object.entries(optionsGroupedByDate).flatMap( ([date,optionListOfSameDate]) => {
            ...
            const _enrichedList = optionListOfSameDate.map(option => {
                ...
                let allPossibleStrategies = putListWithHigherStrikePriceThanStock.reduce( (_allPossibleStrategies, _option) => {
                    ...
                    // if(option.symbol==='ضخود4049' && _option.symbol==='طخود4050'){
                    //     console.log(34)
                    // }
        ...
        let enrichedListOfStock = Object.entries(optionsGroupedByDate).flatMap( ([date,optionListOfSameDate]) => {
            ...
            const _enrichedList = optionListOfSameDate.map(option => {
                ...
                let allPossibleStrategies = putList.reduce( (_allPossibleStrategies, _option) => {
                    ...
                    // if(option.symbol==='ضهرم4020' && _option.symbol==='طهرم4016'){
                    //     console.log(34)
                    // }
    ...
    for (let[stockSymbol,optionList] of Object.entries(optionsGroupedByStock)) {
        ...
        let enrichedListOfStock = Object.entries(optionsGroupedByDate).flatMap( ([date,optionListOfSameDate]) => {
            ...
            const _enrichedList = optionListOfSameDate.map(option => {
                ...
                if(lowStrikePrice===0) return option
                ...
                let allPossibleStrategies = optionListWithHigherStrikePrice.reduce( (_allPossibleStrategies, option2) => {
                    ...
                    let __allPossibleStrategies = optionListWithHigherStrikePrice.reduce( (___allPossibleStrategies, option3) => {
                        ...
                        let strategies = optionListWithHigherStrikePriceThanO3.reduce( (___allPossibleStrategies, option4) => {
                            ...
                            // if(option.symbol==='ضملت5014' && option2.symbol==='ضملت5015' &&  option3.symbol==='ضملت5015' &&  option4.symbol==='ضملت5016'){
                            //     console.log(444);
                            // }
                ...
                let allPossibleStrategies = callListWithHigherStrikePrice.reduce( (_allPossibleStrategies, option2) => {
                    ...
                    let __allPossibleStrategies = putListWithLowerStrikePrice.reduce( (___allPossibleStrategies, put2) => {
                        ...
                        if (showLeftRightProfitType === "LEFT") {
                            // if(option.symbol==='ضستا4031' &&  option2.symbol==='ضستا4032' && putWithSameStrikeOfOption1.symbol==='طستا4031' && put2.symbol==='طستا4028'){
                            //     console.log(333)
                            // }
                            if (maxLeftProfit < 0) {
                                return ___allPossibleStrategies
                    ...
                    let __allPossibleStrategies = optionListWithHigherStrikePrice.reduce( (___allPossibleStrategies, option3) => {
                        ...
                        let strategies = optionListWithHigherStrikePriceThanO3.reduce( (___allPossibleStrategies, option4) => {
                            ...
                            if (BUPS_BEPS_diffStrikesRatio > 1) {
                                ...
                                const BUPS_Gain = ( Math.max(maxGainPrice, option.optionDetails?.strikePrice) - option2.optionDetails?.strikePrice) + BUPS_OpenPositionGain;
            ...
            const _enrichedList = optionListOfSameDate.map(option => {
                ...
                let allPossibleStrategies = optionListWithHigherStrikePrice.reduce( (_allPossibleStrategies, option2) => {
                    ...
                    let __allPossibleStrategies = optionListWithHigherStrikePrice.reduce( (___allPossibleStrategies, option3) => {
                        ...
                        // if(option.symbol==='ضستا0127' && option2.symbol==='ضستا0129' && option3.symbol==='ضستا0128' ){
                        //    console.log([option, option2, option3].map(o=>o.symbol).join(','))
                        // }
            ...
            const _enrichedList = optionListOfSameDate.map(buyingCall => {
                ...
                let allPossibleStrategies = eligiblePutsForBEPS.reduce( (_allPossibleStrategies, sellingPut) => {
                    ...
                    let allPossibleStrategies =  higherStrikePuts.reduce((_allPossibleStrategies, buyingPut) => {
                        ...
                        // if(buyingCall.symbol==='ضهرم6025' && sellingCallWithSameStrikeOfBuyingPut.symbol==='ضهرم6026' && buyingPut.symbol==='طهرم6026' && sellingPut.symbol==='طهرم6025'){
                        //     console.log(343)
                        // }
                        const totalCostWithSign = totalCostCalculator({
...
const RUN = async () => {
    ...
    const strategyMapList = [calcLongGUTS_STRANGLEStrategies(list, {
    ...
    calcBOX_BULL_PUTStrategies(list, {
        priceType: CONSTS.PRICE_TYPE.BEST_PRICE,
        min_time_to_settlement: 6 * 24 * 3600000,
        expectedProfitPerMonth: 1.01,
        expectedProfitNotif: true,
    ...
    calcBOX_BULL_PUTStrategies(list, {
        ...
        priceType: CONSTS.PRICE_TYPE.BEST_PRICE,
        min_time_to_settlement: 12 * 24 * 3600000,
    ...
    htmlContent += strategyMapList.map( ({allStrategiesSorted, htmlTitle, expectedProfitNotif}) => {
        ...
        filteredStrategies = filteredStrategies.filter(strategy => {
            ...
            if (ignoreStrategyList.find(ignoreStrategyObj => {
                ...
                // if(strategy.strategyTypeTitle ==='PUT_BUTTERFLY'){
                //     console.log(33)
                // }
...
const getIgnoreStrategyNames = () => {
    const ignoreListText = document.querySelector('.amin-filter-cnt textarea.amin-ignoreList').value;
    if (!ignoreListText)
        return []
...
const createFilterPanel = () => {
    ...
    cnt.innerHTML = `
        <textarea class="amin-ignoreList"  style="width: 122px;min-width: 122px;height: 200px;font-size: 12px"></textarea>
        <textarea class="amin-filterList"  style="width: 122px;min-width: 122px;height: 200px;font-size: 12px"></textarea>`
    mainCnt.prepend(cnt);
