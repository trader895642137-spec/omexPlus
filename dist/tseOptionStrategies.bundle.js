...
const calcOffsetGainOfPositions = ({ strategyPositions, stockPrice }) => {
    return (0,_common_js__WEBPACK_IMPORTED_MODULE_0__.mainTotalOffsetGainCalculator)({
        strategyPositions,
        getBestPriceCb: (_strategyPosition) => (0,_common_js__WEBPACK_IMPORTED_MODULE_0__.getNearSettlementPrice)({ strategyPosition: _strategyPosition, stockPrice }),
        getReservedMargin: _strategyPosition => {
            return _strategyPosition.margin
        }
    });
}
...
const totalCostCalculator = ({buyOptions, buyStocks, sellOptions, priceType}) => {
    ...
    const totalBuyCost = [buyOptions, buyStocks].filter(Boolean).flatMap(list => list).reduce( (_totalBuyCost, asset) => {
        ...
        return _totalBuyCost + ((price) * (1 + (0,_common_js__WEBPACK_IMPORTED_MODULE_0__.getCommissionFactor)(asset).BUY));
    }
    , 0);
    ...
    const totalSellCost = sellOptions.reduce( (_totalSellCost, asset) => {
        ...
        return _totalSellCost + (price / (1 + (0,_common_js__WEBPACK_IMPORTED_MODULE_0__.getCommissionFactor)(asset).SELL));
    }
    , 0);
...
const IRON_BUTTERFLY_BUPS_strategyObjCreator2 = (option, option2, option3, option4,
    { minStockMiddleDistanceInPercent, maxStockMiddleDistanceInPercent,
        minStockPriceDistanceFromOption4StrikeInPercent, maxStockPriceDistanceFromOption4StrikeInPercent,
        MIN_BUPS_BECS_diffStrikesRatio, MAX_BUPS_BECS_diffStrikesRatio, expectedProfitNotif, priceType, minProfitLossRatio
    }) => {
    ...
    const strategyPositions = [
        ...
        },
        {
            ...
            isSell:true,
            getQuantity:()=> 10,
            getRequiredMargin:()=>diffOfBUPS_Strikes*1000
        },
        {
            ...option3,
            isSell:true,
            getQuantity:()=> 10*BUPS_BECS_diffStrikesRatio,
            getRequiredMargin:()=>diffOfBECS_Strikes*1000
        },
        {
            ...option4,
            isBuy:true,
    ...
    const totalCost = (0,_common_js__WEBPACK_IMPORTED_MODULE_0__.totalCostCalculator)({
        strategyPositions, 
    ...
    const priceThatCauseMaxLoss = Math.max(strategyPositions.map(strategyPosition=>strategyPosition.strikePrice))* 1.1;
    ...
    let maxGainOfButterfly;
    if (diffOfBUPS_Strikes > diffOfBECS_Strikes) {
        let priceThatCauseMaxProfit = option3.optionDetails?.strikePrice;

        maxGainOfButterfly = totalCost + calcOffsetGainOfPositions({strategyPositions, stockPrice:priceThatCauseMaxProfit});

    } else {

        let priceThatCauseMaxProfit = option2.optionDetails?.strikePrice;
                    ...
                    let strategies = optionListBetweenO1AndO2StrikePrice.reduce((__allPossibleStrategies, option3) => {
                        ...
                        if (option3.optionDetails?.strikePrice < option2.optionDetails?.strikePrice) {
                            ...
                            let strategies = optionListWithHigherStrikePriceThanO3.reduce((___allPossibleStrategies, option4) => {
                                ...
                                const strategyObj = IRON_BUTTERFLY_BUPS_strategyObjCreator2(option, option2, option3, option4, {
                                    minStockMiddleDistanceInPercent, maxStockMiddleDistanceInPercent,
                                    minStockPriceDistanceFromOption4StrikeInPercent, maxStockPriceDistanceFromOption4StrikeInPercent,
                                    MIN_BUPS_BECS_diffStrikesRatio, MAX_BUPS_BECS_diffStrikesRatio, expectedProfitNotif, priceType, minProfitLossRatio
                                });
